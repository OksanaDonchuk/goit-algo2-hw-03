...
def build_graph() -> nx.DiGraph:
    ...
    Returns:
        nx.DiGraph: Орієнтований граф із заданими зв'язками та пропускними здатностями.
    """
    G = nx.DiGraph()
...
def compute_max_flow(G: nx.DiGraph, sources: List[str], sinks: List[str]) -> Tuple[int, Dict[str, Dict[str, int]]]:
    ...
    Args:
        G (nx.DiGraph): Граф логістичної мережі.
        sources (List[str]): Список терміналів (джерел потоку).
        sinks (List[str]): Список магазинів (стоків потоку).
    
    Returns:
        Tuple[int, Dict[str, Dict[str, int]]]:
            - Максимальне значення потоку.
            - Словник із розподілом потоку по ребрах.
    """
...
def calculate_terminal_to_store_flows(flow_dict: Dict[str, Dict[str, int]], sources: List[str], intermediate_nodes: List[str], sinks: List[str]) -> Dict[Tuple[str, str], int]:
    ...
    Args:
        flow_dict (Dict[str, Dict[str, int]]): Розподіл потоку по ребрах.
        sources (List[str]): Список терміналів.
        intermediate_nodes (List[str]): Список проміжних вузлів (складів).
        sinks (List[str]): Список магазинів.
    
    Returns:
        Dict[Tuple[str, str], int]: Потоки між терміналами та магазинами.
    """
